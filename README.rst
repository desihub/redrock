...
**1) Without Archetypes (default redrock)**::
...
Example run (with all the default values, i.e. in per camera mode with priors)::

    rrdesi -i <spectra_file> --archetypes <archetype_dir or archetype_file> -o <output_file> -d <details_file.h5> --archetype-legendre-percamera False
...
Example run (only with archetypes, without any Legendre polynomials)::
